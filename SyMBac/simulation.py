...
class Simulation:
    ...
    def __init__(self, trench_length, trench_width, cell_max_length, max_length_var, cell_width, width_var, lysis_p, sim_length, pix_mic_convs, gravity, phys_iters, resize_amount, save_dir):
        """
        ...
        self.pix_mic_conv = np.min(pix_mic_convs)
        if type(pix_mic_convs) in [int, float, np.float_, np.int_]:
            self.pix_mic_convs = np.array([pix_mic_convs])
        elif type(pix_mic_convs) in [list, tuple, np.ndarray]:
            self.pix_mic_convs = np.sort(pix_mic_convs)
        else:
            print(type(pix_mic_convs), "is not a valid datatype for pix_mic_convs")
            raise TypeError
        ...
        self.OPL_scenes = None
        self.masks = None
        self.main_segments = None
        self.cell_timeseries_properties = None
    ...
    def draw_simulation_OPL(self, do_transformation = True, label_masks = True, return_output = False):
        ...
        if len(self.pix_mic_convs) == 1:
            ...
            space_size = (int(np.ceil(space_size[0]/100))*100,int(np.ceil(space_size[1]/100))*100)
            ...
            self.cell_timeseries_properties = Parallel(n_jobs=-1)(
                    delayed(gen_cell_props_for_draw)(a, ID_props) for a in tqdm(self.cell_timeseries, desc='Timeseries Properties'))
            space_size_base = get_space_size(self.cell_timeseries_properties)
            factor = int(0.655/self.pix_mic_conv*10)
            space_size_base = (int(np.ceil(space_size_base[0]/factor))*factor,int(np.ceil(space_size_base[1]/factor))*factor)
            scenes = Parallel(n_jobs=-1)(delayed(draw_scene)(
                cell_properties, do_transformation, space_size_base, self.offset, label_masks) for cell_properties in tqdm(
                    self.cell_timeseries_properties, desc='Scene Draw:'))
            OPL_scenes = [_[0] for _ in scenes]
            masks = [_[1] for _ in scenes]
            temp_sim = Simulation(trench_length=self.trench_length,
                                    trench_width=self.trench_width,
                                    cell_max_length=self.cell_max_length,
                                    max_length_var=self.max_length_var,
                                    cell_width=self.cell_width,
                                    width_var=self.width_var,
                                    lysis_p=self.lysis_p,
                                    sim_length=10,
                                    pix_mic_convs=self.pix_mic_conv,
                                    gravity=self.gravity,
                                    phys_iters=self.phys_iters,
                                    resize_amount=self.resize_amount,
                                    save_dir=self.save_dir)
            temp_sim.run_simulation(show_window=False)
            main_segments = get_trench_segments(temp_sim.space)
            for pmc in self.pix_mic_convs:
                print("pmc", pmc)
                new_sim = Simulation(trench_length=self.trench_length,
                                    trench_width=self.trench_width,
                                    cell_max_length=self.cell_max_length,
                                    max_length_var=self.max_length_var,
                                    cell_width=self.cell_width,
                ...
                #                     save_dir=self.save_dir)
                # temp_sim.run_simulation(show_window=False)
                new_sim.main_segments = main_segments
                if pmc == self.pix_mic_conv:
                    new_sim.cell_timeseries_properties = self.cell_timeseries_properties
                else:
                    new_sim.cell_timeseries_properties = self.update_timeseries_properties(pmc)

                space_size = (int(space_size_base[0]*self.pix_mic_conv/pmc),int(space_size_base[1]*self.pix_mic_conv/pmc))
                new_sim.offset = np.min([30, space_size[1]//2])

                new_sim.OPL_scenes = [linear_interpolate(OPL_scene,space_size) for OPL_scene in OPL_scenes]
                new_sim.masks = [linear_interpolate(mask,space_size) for mask in masks]

                self.simulations_list.append(new_sim)
            return self.simulations_list
            w

    def update_timeseries_properties(self, pmc):
        new_timeseries = []
        for timepoint in self.cell_timeseries_properties:
            tp = []
            for cell in timepoint:
                new_cell = [cell[0]*self.pix_mic_conv/pmc,  # cell length
                            cell[1]*self.pix_mic_conv/pmc,  # cell width
                            cell[2],                        # cell angle
                            cell[3]*self.pix_mic_conv/pmc,  # cell centroid coordinates
                            cell[4],                        # frequency modif
                            cell[5],                        # amplitude modif
                            cell[6],                        # phase modif
                            cell[7],                        # phase mult
                            cell[8]*self.pix_mic_conv/pmc]  # pinching separation
                tp.append(new_cell)
            new_timeseries.append(tp)
