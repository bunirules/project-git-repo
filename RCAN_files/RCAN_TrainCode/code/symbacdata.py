...
class SRCNNDataset(Dataset):
    ...
    def __getitem__(self, index):
        # print(f"the index is: {index}, total length is: {len(self.all_label_paths)}")
        ...
        # label = np.vstack([label,label,label])
...
def get_dataloaders(dataset_train, dataset_valid):
    print(f"the train batch size is {TRAIN_BATCH_SIZE}, according to me")
